...
open class AccountController (
    ...
    @ApiResponses(
        ApiResponse(code = 200, message = "The transfer between the source account and the target was made"),
        ApiResponse(
            code = 400,
            message = "The source account doesn't has enough funds as specified in the amount, or either invalid input parameters",
            response = ErrorPayload::class
        ),
        ApiResponse(
            code = 404,
            message = "Either the source or the target couldn't be found.",
            response = ErrorPayload::class
        ),
    ...
    fun transferFundsToAccount(@RequestBody payload: @Valid MoneyTransferRequestPayload): Mono<ResponseEntity<Void>> {
        ...
        return accountTransferService.transferMoneyFromAccountToAnotherAccount(
    ...
    @ApiResponses(
        ApiResponse(code = 200, message = "The account details for the given account id."),
    ...
    @GetMapping("/{id}")
    fun getAccountDetails(@PathVariable("id") id: @NotBlank @Size(max = 100) @Pattern(regexp = "^[a-zA-Z0-9\\s]*$") String): Mono<AccountPayload> {
